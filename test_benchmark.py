...
if torch.cuda.is_available():
    model = model.cuda()
model.load_state_dict(torch.load('autodl-tmp/epochs/' + MODEL_NAME))

test_set = TestDatasetFromFolder('autodl-tmp/test', upscale_factor=UPSCALE_FACTOR)
test_loader = DataLoader(dataset=test_set, num_workers=4, batch_size=1, shuffle=False)
test_bar = tqdm(test_loader, desc='[testing benchmark datasets]')

out_path = 'autodl-tmp/benchmark_results/SRF_' + str(UPSCALE_FACTOR) + '/'
